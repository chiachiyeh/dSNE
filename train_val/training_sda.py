...
from utils.datasets_su import DomainFolderDataset, DomainRecDataset, DomainArrayDataset, DomainNumpyDataset
...
class DomainModel(object):
    ...
    def create_final_dataset(self, train_tforms, eval_tforms):
        ...
        :peram train_tforms: training transformers
        :param eval_tforms: evaluation transformers
        :return:
            trs_set: training source set
            trt_set: training target set
        ...
        if self.args.aug_tgt_only:
            trs_set = DomainNumpyDataset(Sx, Sy=Sy, tforms=train_tforms)
        else:
            trs_set = DomainNumpyDataset(Sx, Sy=Sy, tforms=eval_tforms)
        trt_set = DomainNumpyDataset(Tx, tforms=train_tforms)
...
class AuxModel(DomainModel):
    ...
    def create_final_dataset(self, train_tforms, eval_tforms):
        ...
        trs_set = DomainNumpyDataset(Sx, Sy=Sy, Tx=Tx, Ty=Ty, tforms = train_tforms, ratio=self.args.ratio)
        tes_set = DomainNumpyDataset(Sx, Sy=Sy, tforms=eval_tforms)
        tet_set = DomainNumpyDataset(Tx, tforms=eval_tforms)
